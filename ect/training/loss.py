...
class ECMLoss:
    def __init__(self, P_mean=-1.1, P_std=2.0, sigma_data=0.5, q=4, c=0.0, k=8.0, b=1.0, cut=4.0, adj='sigmoid'):
        self.P_mean = P_mean
        self.P_std = P_std
    ...
    def __call__(self, net, images, map, labels=None, augment_pipe=None):
        ...
        D_yt = net(y + eps_t, map, t, labels, augment_labels=augment_labels)
        
        if r.max() > 0:
            torch.cuda.set_rng_state(rng_state)
            with torch.no_grad():
                D_yr = net(y + eps_r, map, r, labels, augment_labels=augment_labels)
        ...
        # L1 Loss
        # loss = torch.abs(D_yt - D_yr)  # L1
        # loss = torch.sum(loss.view(loss.shape[0], -1), dim=-1)/(64*64*3) 
