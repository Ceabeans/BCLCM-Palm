...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        channel_mult        = [1,2,2,2,4],    # Per-resolution multipliers for the number of channels.
...
class ECMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        **model_kwargs,                     # Keyword arguments for the underlying model.
        ...
        self.sigma_max = sigma_max
        self.sigma_data = sigma_data
        self.model = globals()[model_type](img_resolution=img_resolution, in_channels=img_channels, out_channels=img_channels-1, label_dim=label_dim, **model_kwargs)

    def forward(self, x, map, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        x = x.to(torch.float32)
...
def generator_fn(
    ...
    # 兼容 mid_t 是 float、int 或 list
    if mid_t is None:
        mid_t_list = []
    elif isinstance(mid_t, (float, int)):
        mid_t_list = [mid_t]
    else:
        mid_t_list = list(mid_t)

    # Time step discretization
    t_steps = torch.tensor([t_max] + mid_t_list, dtype=torch.float64, device=latents.device)
    t_steps = torch.cat([t_steps, torch.zeros_like(t_steps[:1])])

    # Sampling
    x = latents.to(torch.float64) * t_steps[0]
    map = map.to(torch.float64)

    one_step = None
    two_step = None

    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
        x = net(x, map, t_cur, class_labels).to(torch.float64)

        # 用 item() + 误差容忍比较
        if abs(t_cur.item() - t_max) < 1e-5:
            one_step = x.clone()

        if any(abs(t_cur.item() - m) < 1e-5 for m in mid_t_list):
            two_step = x.clone()

        if t_next > 0:
            x = x + t_next * torch.randn_like(x)

    return one_step.to(torch.float32), two_step.to(torch.float32)